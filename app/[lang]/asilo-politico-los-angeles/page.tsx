...
const PAGE_SLUG = 'asilo-politico-los-angeles';
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title, description, url: `${SITE_URL}/${lang}/${PAGE_SLUG}`, type: 'website', siteName: 'Manuel Solís Law Firm', locale: isEs ? 'es_US' : 'en_US', images: [{ url: `${SITE_URL}/home-image.jpg`, width: 1200, height: 630, alt: title }] },
    twitter: { card: 'summary_large_image', title, description, images: [`${SITE_URL}/home-image.jpg`], creator: '@AbogadoMSolis' },
...
export default async function Page({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const legalServiceSchema = {
    ...
    name: `Manuel Solis - ${service.title[currentLang]}`, description: config.metaDescription[currentLang],
    url: `${SITE_URL}/${currentLang}/${PAGE_SLUG}`, telephone: office.phone,
