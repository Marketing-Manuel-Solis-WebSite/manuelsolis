...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    ...buildSocialMetadata({
      ...
      title,
      description,
      images: [{ url: '/og/oficina-league-city.jpg', width: 1000, height: 1000 }],
    }),
  };
}
...
export default async function LeagueCityPage({ params }: Props) {
  ...
  const officeFaqs = buildMainOfficeFaqs('league-city', lang === 'en' ? 'en' : 'es');
  const faqSchema = buildFaqPageSchema(
    officeFaqs,
    `https://www.manuelsolis.com/${lang}/oficinas/league-city`,
  );
