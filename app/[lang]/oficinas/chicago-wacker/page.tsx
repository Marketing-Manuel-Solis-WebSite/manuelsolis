...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = isEs
    ? `Abogados en Chicago (S Wacker Dr)`
    : `Lawyers in Chicago (S Wacker Dr)`;
  ...
  const schemaData = await buildOfficeSchema(
    {
      ...
      description: {
        es: `Dirección de Manuel Solís en ${nap.city} (S Wacker Dr) que se atiende solo con cita previa, sin personal del despacho en el sitio. Inmigración, familia y accidentes.`,
        en: `Manuel Solis by-appointment location in ${nap.city} (S Wacker Dr), with no firm staff on site. Immigration, family law, and accident cases.`,
      },
    },
    localeLang,
  );
