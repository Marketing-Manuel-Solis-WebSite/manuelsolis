...
/**
 * Category: Protección Legal para Migrantes — server-first (Fase 2.3 blog).
 * Purely presentational (no interactive state), so it's a pure Server
 * Component: the bilingual `texts` resolve to the active `lang` on the server
 * and the inactive locale never reaches the client bundle (enfoque b). The
 * animated background orbs + scrolling word are now static (matching the
 * site-wide conversion); the article grid enters via Stagger and uses
 * `.card-3d`. page.tsx (generateMetadata) is untouched — only the `lang` prop
 * is threaded in.
 */
