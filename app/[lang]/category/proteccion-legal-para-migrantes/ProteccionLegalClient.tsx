...
/**
 * Category: Protección Legal para Migrantes — server-first (Fase 2.3 blog). El
 * listado se deriva de BLOG_DATA (fuente única) filtrando por `categoryId`, así
 * que cada tarjeta apunta a un post vivo (/{lang}/blog/{slug}). Purely
 * presentational (no interactive state), so it's a pure Server Component: the
 * bilingual `copy` resolves to the active `lang` on the server and the inactive
 * locale never reaches the client bundle (enfoque b). Las tarjetas son el mismo
 * <BlogCard> del índice del blog; el grid entra con Stagger.
 */

// IDs de BLOG_DATA.categories que alimentan esta categoría: las protecciones
// humanitarias (Visa U, VAWA, Visa T, asilo y SIJS).
const CATEGORY_IDS = ['visa-u', 'visa-VAWA', 'visa-T', 'visa-humanitaria'];

const copy = {
  library: { es: 'Biblioteca Legal', en: 'Legal Library' },
  title: { es: 'Protección Legal para Migrantes', en: 'Legal Protection for Migrants' },
  subtitle: {
    es: 'Guías sobre las protecciones humanitarias que pueden dar estatus a víctimas y solicitantes: Visa U, VAWA, Visa T, asilo y estatus juvenil (SIJS).',
    en: 'Guides on the humanitarian protections that can grant status to victims and applicants: U Visa, VAWA, T Visa, asylum, and juvenile status (SIJS).',
  },
  ...
  articles: { es: 'Artículos sobre protección legal', en: 'Articles on legal protection' },
  viewAll: { es: 'Ver todos los artículos', en: 'View all articles' },
};

// --- COMPONENTE DE PÁGINA ---
export default function ProteccionLegalClient({ lang }: { lang: 'es' | 'en' }) {
