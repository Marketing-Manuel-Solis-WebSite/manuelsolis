...
/**
 * Client island: interactive Visa U info tabs (pre-resolved data). Every panel is
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <m.button key={tab.id} onClick={() => setSelectedTab(tab.id)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                aria-expanded={on} aria-controls={`visa-u-panel-${tab.id}`}
        ...
        <div className="max-w-5xl mx-auto">
          {tabs.map((tab) => {
            ...
            return (
              <m.div key={tab.id} id={`visa-u-panel-${tab.id}`} hidden={!on} initial={false}
