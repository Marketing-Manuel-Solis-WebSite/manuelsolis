...
import { Scale, MapPin, PhoneCall, ArrowRight, Zap, Truck, Car, Stethoscope, FileText, HandCoins } from 'lucide-react';
import { Reveal } from '../../../components/motion';
import type { ResolvedCase, ResolvedUi, IconKey } from './segurosData';

const ICONS: Record<IconKey, ElementType> = { zap: Zap, truck: Truck, car: Car, stethoscope: Stethoscope, scale: Scale, fileText: FileText, handCoins: HandCoins };

/**
 * Client island: interactive "Legal Solutions in Insurance" tabs (pre-resolved
 * data). Every panel is rendered (server HTML included) and the inactive ones are
 * hidden with the `hidden` attribute, so the unique copy of each tab stays
 * crawlable.
 */
export default function SegurosCases({ cases, ui }: { cases: ResolvedCase[]; ui: ResolvedUi }) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          {cases.map((service) => {
            ...
            return (
              <m.button key={service.id} onClick={() => setSelectedTab(service.id)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                aria-expanded={on} aria-controls={`seguros-panel-${service.id}`}
        ...
        <div className="max-w-5xl mx-auto">
          {cases.map((service) => {
            ...
            return (
              <m.div key={service.id} id={`seguros-panel-${service.id}`} hidden={!on} initial={false}
