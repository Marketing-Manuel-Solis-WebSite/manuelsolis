...
import { ArrowRight, Scale, Lock, ShieldAlert, Gavel, BookOpen, Target, Shield } from 'lucide-react';
import type { ResolvedTab, TabIconKey } from './defensaData';
import type { Language } from '../../../lib/translations';

const ICONS: Record<TabIconKey, ElementType> = { scale: Scale, lock: Lock, shieldAlert: ShieldAlert, gavel: Gavel, bookOpen: BookOpen };

/**
 * Client island: interactive Deportation Defense tabs (pre-resolved data). Every
 ...
 */
export default function DeportacionCases({ tabs, requestEvaluation, detainedLink, lang }: { tabs: ResolvedTab[]; requestEvaluation: string; detainedLink: string; lang: Language }) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <m.button key={tab.id} onClick={() => setSelectedTab(tab.id)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                aria-expanded={on} aria-controls={`deportacion-panel-${tab.id}`}
        ...
        <div className="max-w-5xl mx-auto">
          {tabs.map((tab) => {
            ...
            return (
              <m.div key={tab.id} id={`deportacion-panel-${tab.id}`} hidden={!on} initial={false}
                ...
                <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 md:p-12 border border-white/10 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="space-y-10">
                    ...
                    {tab.id === 'detenidos' && (
                      <div>
                        <a href={`/${lang}/clientes-detenidos`} className="inline-flex items-center gap-3 px-6 py-4 bg-red-500/10 border border-red-500/30 rounded-xl text-red-300 hover:text-white hover:bg-red-500/20 transition-all font-bold group">
                          <ShieldAlert size={20} />
                          {detainedLink}
