...
import { mainCases, ui, getText } from './immigrationData';
import type { Language } from '../../../lib/translations';

/**
 * Client island: the interactive "Legal Solutions" tabs. Holds the selected-tab
 * state; every panel is rendered (server HTML included) and the inactive ones are
 * hidden with the `hidden` attribute, so the unique copy of each tab stays
 * crawlable. Server-rendered text passes through the shared data module.
          ...
          {mainCases.map((service) => {
            ...
            return (
              <m.button
                ...
                aria-controls={`inmigracion-panel-${service.id}`}
        ...
        <div className="max-w-5xl mx-auto">
          {mainCases.map((service) => {
            ...
            return (
              <m.div
                key={service.id}
                id={`inmigracion-panel-${service.id}`}
                hidden={!on}
                initial={false}
