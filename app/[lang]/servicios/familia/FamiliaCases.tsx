...
import { Scale, MapPin, PhoneCall, ArrowRight, FileText, MessageSquare, HandCoins } from 'lucide-react';
import { Reveal } from '../../../components/motion';
import type { ResolvedCase, ResolvedUi, IconKey } from './familiaData';

const ICONS: Record<IconKey, ElementType> = { fileText: FileText, messageSquare: MessageSquare, handCoins: HandCoins, phoneCall: PhoneCall, scale: Scale };

/**
 * Client island: interactive "Solutions in Family Law" tabs (pre-resolved data).
 * Every panel is rendered (server HTML included) and the inactive ones are hidden
 * with the `hidden` attribute, so the unique copy of each tab stays crawlable.
 */
export default function FamiliaCases({ cases, ui }: { cases: ResolvedCase[]; ui: ResolvedUi }) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          {cases.map((service) => {
            ...
            return (
              <m.button key={service.id} onClick={() => setSelectedTab(service.id)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                aria-expanded={on} aria-controls={`familia-panel-${service.id}`}
        ...
        <div className="max-w-5xl mx-auto">
          {cases.map((service) => {
            ...
            return (
              <m.div key={service.id} id={`familia-panel-${service.id}`} hidden={!on} initial={false}
