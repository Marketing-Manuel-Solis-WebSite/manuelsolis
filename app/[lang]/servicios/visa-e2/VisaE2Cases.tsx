...
import { ArrowRight, FileText, CheckCircle2, Shield, Globe, Target, DollarSign } from 'lucide-react';
import type { ResolvedTab, IconKey } from './visaE2Data';

const ICONS: Record<string, ElementType> = { fileText: FileText, checkCircle2: CheckCircle2, shield: Shield, globe: Globe };

/**
 * Client island: interactive E-2 info tabs (pre-resolved data). Every panel is
 ...
 */
export default function VisaE2Cases({ tabs, requestEvaluation }: { tabs: ResolvedTab[]; requestEvaluation: string }) {
    ...
    <section className="px-4 py-24 relative z-10 bg-[#001026]" id="detalles">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="max-w-5xl mx-auto">
          {tabs.map((tab) => {
            const on = selectedTab === tab.id;
            const TabIcon = ICONS[tab.iconKey as IconKey] || FileText;
            return (
              <m.div key={tab.id} id={`visa-e2-panel-${tab.id}`} hidden={!on} initial={false}
