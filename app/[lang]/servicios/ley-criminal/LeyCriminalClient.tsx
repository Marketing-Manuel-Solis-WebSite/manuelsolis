...
import { PhoneCall, ArrowRight, FileText, Star, Scale, CheckCircle2 } from 'lucide-react';
import Header from '../../../components/Header';
import Footer from '../../../components/Footer';
import ContactForm from '../../../components/ContactForm';
import Breadcrumbs from '../../../components/Breadcrumbs';
import { Reveal, Stagger, StaggerItem, MagneticButton } from '../../../components/motion';
import LeyCriminalCases from './LeyCriminalCases';
import { resolveCases, resolveSteps, resolveUi, type IconKey } from './leyCriminalData';
import type { Language } from '../../../lib/translations';

const ICONS: Record<IconKey, ElementType> = { phoneCall: PhoneCall, fileText: FileText, scale: Scale, checkCircle2: CheckCircle2, messageSquare: FileText, zap: FileText, car: FileText };

/**
 * Criminal Law page — server-first (Fase 2.3 Lote A, enfoque b). Bilingual data
 * resolved to the active locale on the server (immigrationData-style module),
 * islands receive single-locale strings. LCP sacred: criminal-hero.png priority
 * + H1 static. page.tsx generateMetadata + JSON-LD untouched.
 */
export default function LeyCriminalClient({ lang }: { lang: Language }) {
  const isEs = lang === 'es';
    ...
    <div className="min-h-screen flex flex-col bg-navy-500 text-white relative selection:bg-[#B2904D] selection:text-white font-sans overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none w-full h-full transform-gpu">
        ...
        <div aria-hidden="true" className="absolute inset-0 flex items-center justify-center opacity-[0.02] pointer-events-none select-none overflow-hidden">
          <span className="text-[120vh] font-black italic text-white tracking-tighter whitespace-nowrap">LEY CRIMINAL</span>
        </div>
      </div>
      ...
      <main id="main-content" tabIndex={-1}>
        <div className="relative z-10 pt-24 md:pt-28 px-4">
          <div className="container mx-auto max-w-7xl">
            <Breadcrumbs items={[
              ...
              { label: { es: 'Defensa Criminal', en: 'Criminal Defense' }, href: `/${lang}/servicios/ley-criminal` },
            ]} />
          </div>
        </div>
              ...
              <div className="lg:col-span-5 relative h-[300px] sm:h-[400px] md:h-[600px] lg:h-[700px] flex items-center justify-center order-2 lg:order-1">
                ...
                <div className="relative z-10 w-full h-full flex items-center justify-center transform-gpu">
                  <div className="relative w-full h-full">
                    <Image src="/criminal-hero.png" alt="Abogado de defensa criminal en Houston, Texas" fill className="object-contain object-center drop-shadow-[0_0_20px_rgba(56,189,248,0.5)]" priority sizes="(max-width: 768px) 100vw, 50vw" />
                  </div>
                </div>
        ...
        <LeyCriminalCases cases={cases} ui={ui} />
