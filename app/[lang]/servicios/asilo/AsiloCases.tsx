...
import { ArrowRight, Shield, Scale, Flag, AlertTriangle, ShieldCheck, Target, BookOpen } from 'lucide-react';
import type { ResolvedTab, TabIconKey } from './asiloData';

const ICONS: Record<TabIconKey, ElementType> = { shield: Shield, scale: Scale, flag: Flag, alertTriangle: AlertTriangle, shieldCheck: ShieldCheck };

/**
 * Client island: interactive Asylum info tabs (pre-resolved data). Handles
 * subPoints both with and without a subTitle. Every panel is rendered (server
 * HTML included) and the inactive ones are hidden with the `hidden` attribute,
 * so the unique copy of each tab stays crawlable.
 */
export default function AsiloCases({ tabs, requestEvaluation }: { tabs: ResolvedTab[]; requestEvaluation: string }) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <m.button key={tab.id} onClick={() => setSelectedTab(tab.id)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                aria-expanded={on} aria-controls={`asilo-panel-${tab.id}`}
        ...
        <div className="max-w-5xl mx-auto">
          {tabs.map((tab) => {
            ...
            return (
              <m.div key={tab.id} id={`asilo-panel-${tab.id}`} hidden={!on} initial={false}
