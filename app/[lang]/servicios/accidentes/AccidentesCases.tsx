...
import { allServices, ui, getText } from './accidentesData';
 ...
 * service.offices son strings (ALL_OFFICES) idénticos en ambos idiomas.
 */
 ...
 * selected-tab state; every panel is rendered (server HTML included) and the
 * inactive ones are hidden with the `hidden` attribute, so the unique copy of
 * each tab stays crawlable. Conditional content (quotes for trailer, benefits
 * for trabajo, solution for medica/explosion/auto) preserved 1:1.
  ...
  return (
    <section className="px-4 pb-32 relative z-10" id="casos">
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          {allServices.map((service) => {
            ...
            return (
              <m.button
                ...
                aria-controls={`accidentes-panel-${service.id}`}
        ...
        {/* DETAIL — all panels in the HTML, inactive ones hidden */}
        <div className="max-w-5xl mx-auto">
          {allServices.map((service) => {
            const on = selectedTab === service.id;
            const CaseIcon = service.icon;
            return (
              <m.div key={service.id} id={`accidentes-panel-${service.id}`} hidden={!on} initial={false}
                ...
                <div className="bg-white/5 backdrop-blur-md rounded-2xl sm:rounded-[3rem] p-5 sm:p-8 md:p-12 border border-white/10 shadow-2xl">
                  <div className="flex items-start gap-6 mb-8 pb-8 border-b border-white/10">
                    ...
                    <div className="flex-1">
                      <h3 className="text-3xl md:text-4xl font-black text-white mb-3 leading-tight">{gT(service.title)}</h3>
                      <p className="text-[#B2904D] text-sm font-bold uppercase tracking-widest mb-4">{gT(service.subtitle)}</p>
                      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 border border-white/20">
                        ...
                        <span className="text-xs text-white/80 font-medium">{service.offices.length} {t('officesCount')}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-8">
                    <div>
                      <h4 className="text-2xl font-black text-white mb-4">{gT(service.content.intro)}</h4>
                      <p className="text-lg text-white/70 leading-relaxed">{gT(service.content.description)}</p>
                    </div>
                        ...
                        {service.offices.map((office, i) => {
                          ...
                          return (
                            <Link
                              ...
                              aria-label={`${gT(service.title)} — ${office}`}
                    ...
                    {service.id === 'trailer' && service.content.quotes && (
                      <div className="space-y-4">
                        {service.content.quotes.map((quote, i) => (
                    ...
                    {service.id === 'trabajo' && service.content.benefits && (
                      <div className="bg-white/5 p-8 rounded-2xl border border-white/10">
                        <h5 className="font-black text-white mb-6 flex items-center gap-3 text-xl">
                          ...
                          {gT(service.content.benefitsTitle)}
                        </h5>
                        ...
                        <p className="text-white/60 text-sm italic">{gT(service.content.closing)}</p>
                      </div>
                    )}

                    {service.content.solution && (service.id === 'medica' || service.id === 'explosion' || service.id === 'auto') && (
                      <div className="bg-white/5 p-8 rounded-2xl border border-white/10">
                        <p className="text-white/80 leading-relaxed font-medium text-lg">{gT(service.content.solution)}</p>
                      </div>
                    )}
