...
export default async function AccidentesPage({ params }: Props) {
  ...
  const serviceAttorneys = getServiceAttorneys('accidentes', lang === 'en' ? 'en' : 'es');
  ...
  return (
    <>
      ...
      <AccidentesClient lang={lang === 'en' ? 'en' : 'es'} faqs={serviceFaqs} serviceAttorneys={serviceAttorneys} />
    </>
  );
}
